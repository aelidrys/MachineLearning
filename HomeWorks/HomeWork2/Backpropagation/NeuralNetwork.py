...
class NeuralNetwork:
    def __init__(self, hidden_layer_weights, output_layer_weights, activation_type='poly', lr=0.5):
    ...
    def compute_delta(self, target):
        ...
        for i, neuron in enumerate(self.output_layer.neurons):
            ...
            self.dE_dO_net[i] = self.dE_dO_out[i] * neuron.activation_derv()
        ...
        for i, neuron in enumerate(self.hidden_layer.neurons):
            self.dE_dH_out[i] = np.dot(self.dE_dO_net, self.output_layer.weights[:,i])
            self.dE_dH_net[i] = self.dE_dH_out[i] * neuron.activation_derv()
        print(f'\tdE_dH_net = {self.dE_dH_net}\n')

    def update_weights(self):
        # Update output layer
        for i, neuron in enumerate(self.output_layer.neurons):
            for j, wight in enumerate(self.output_layer.weights[i]):
                dE_dW = self.dE_dO_net[i] * neuron.input[j]
                self.output_layer.weights[i][j] = wight - dE_dW * self.lr
                new_w = self.output_layer.weights[i][j]
                print(f"\to: inputs[{j}] = {neuron.input[j]:.2f}, wight = {wight:.2f}, delta = {dE_dW:.4f}, new_w = {new_w:.4f}")
            
            
        # Update hidden layer
        for i, neuron in enumerate(self.hidden_layer.neurons):
            for j, wight in enumerate(self.hidden_layer.weights[i]):
                dE_dW = self.dE_dH_net[i] * neuron.input[j]
                self.hidden_layer.weights[i][j] = wight - dE_dW * self.lr
                new_w = self.hidden_layer.weights[i][j]
                print(f"\th: inputs[{j}] = {neuron.input[j]:.2f}, wight = {wight:.2f}, delta = {dE_dW:.4f}, new_w = {new_w:.4f}")
...
if __name__ == '__main__':
    ...
    if len(argv) > 1:
        activation = argv[1]
    print(f"Activation fuction: {activation}\n")
    if activation == "poly":
        poly()
